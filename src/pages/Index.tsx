...
import { useRelayKindCounts, kindLabel, GIT_KINDS } from '@/hooks/useRelayKindCounts';
...
function groupKindCounts(counts: Record<number, number>): Array<[string, { n: number; kinds: number[] }]> {
  const grouped = Object.entries(counts).reduce<Record<string, { n: number; kinds: number[] }>>((acc, [k, n]) => {
    const label = kindLabel(Number(k));
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
      ...
      {chips.map(([label, { n, kinds }]) => (
        <span
          key={label}
          title={`kind${kinds.length > 1 ? 's' : ''} ${kinds.join(', ')}`}
          className="text-[9px] font-mono text-muted-foreground/60"
        >
          <span className="text-foreground/70">{label}</span>
          <span className="text-muted-foreground/40">×</span>
          <span className="text-foreground/80 tabular-nums">{n}</span>
        </span>
      ))}
    </div>
  );
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-6 py-10 space-y-8">
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-2">
            <AnimatedTitle accent="pls" rest="" from="#FF7849" to="#FFB347" suffixRgba="rgba(255,120,73,0.2)" />
            <NstartHand />
          </div>
